...
// Progress helper
// ---------------------------------------------------------------------------
async function fetchWithProgress(url, baseProgress, maxProgress) {
...
self.addEventListener('message', async (event) => {
    ...
    if (type === 'init') {
        try {
            ...
            // WebGPU has an 8 storage-buffer limit per stage; several ONNX ops
            // (Concat, Split, Slice, …) need more than that in the current
            // onnxruntime-web implementation, so we force WASM for every model.
            const buf1 = await fetchWithProgress(BUBBLE_MODEL_PATH, 0, 25);
            bubbleSession = await ort.InferenceSession.create(buf1, {
                executionProviders: ['wasm'],
                graphOptimizationLevel: 'all'
            });
            console.log("[Worker] Bubble detector loaded (wasm)");

            const buf2 = await fetchWithProgress(PANEL_MODEL_PATH, 25, 50);
            panelSession = await ort.InferenceSession.create(buf2, {
                executionProviders: ['wasm'],
                graphOptimizationLevel: 'all'
            });
            console.log("[Worker] Panel detector loaded (wasm)");

            try {
                const buf3 = await fetchWithProgress(PANEL_ORDER_PATH, 50, 75);
                panelOrderSession = await ort.InferenceSession.create(buf3, {
                    executionProviders: ['wasm'],
                    graphOptimizationLevel: 'all'
                });
                console.log("[Worker] Panel order model loaded (wasm)");
            } catch (e) {
                console.warn("[Worker] Failed to load panel order model:", e.message);
            }

            try {
                const buf4 = await fetchWithProgress(READERNET_PATH, 75, 100);
                readernetSession = await ort.InferenceSession.create(buf4, {
                    executionProviders: ['wasm'],
                    graphOptimizationLevel: 'all'
                });
                console.log("[Worker] ReaderNet Poneglyph loaded (wasm)");
            } catch (e) {
                console.warn("[Worker] Failed to load ReaderNet Poneglyph:", e.message);
            }

            console.log("[Worker] All detection models loaded");
            self.postMessage({ status: 'ready' });
        } catch (err) {
...
// ---------------------------------------------------------------------------
// Bubble detector preprocessing (unchanged)
// ---------------------------------------------------------------------------
...
async function detectPanels(bitmap) {
    ...
    // Get input dims safely (fallback to 800x800, the training imgsz)
    let inH = 800, inW = 800;
    try {
    ...
    // Resize and pad (letterbox to match training preprocessing)
    ...
    // Safely grab the first output tensor
    const outKeys = Object.keys(results);
    if (!outKeys.length) {
        console.error("[Worker] Panel detector returned no outputs");
        return [];
    }
    const output = results[outKeys[0]];
    if (!output?.dims) {
        console.error("[Worker] Panel detector output missing dims:", outKeys);
        return [];
    }

    // YOLO-pose raw output can be [1, C, 8400] or [1, 8400, C]
    const d = output.dims;
    ...
    if (d.length === 3) {
        ...
        console.error("[Worker] Unexpected panel detector output shape:", d);
        return [];
    }
    ...
    for (let col = 0; col < numPreds; col++) {
        ...
        // YOLO exports from Ultralytics output pixel coords in [0, input_size]
        const x1 = cx - w / 2;
        const y1 = cy - h / 2;
        const x2 = cx + w / 2;
        const y2 = cy + h / 2;
    ...
    // Scale back to original image (subtract letterbox padding, divide by scale)
    const panels = [];
    for (const p of keep) {
...
// ---------------------------------------------------------------------------
// Panel ordering (pairwise ResNet model)
// ---------------------------------------------------------------------------
async function rankPanels(panels, bitmap) {
    ...
    // Extract panel crops (RGB, 224x224, ImageNet normalized)
    const crops = [];
    const validPanels = [];
    ...
    if (validPanels.length <= 1) return validPanels.length > 0 ? validPanels : panels;
    ...
    for (let start = 0; start < imgAList.length; start += batchSize) {
        ...
        const imgA = new Float32Array(batchCount * 3 * PANEL_CROP_SIZE * PANEL_CROP_SIZE);
        const imgB = new Float32Array(batchCount * 3 * PANEL_CROP_SIZE * PANEL_CROP_SIZE);
        const pos = new Float32Array(batchCount * 12);
        ...
        const feed = {
            [panelOrderSession.inputNames[0]]: new ort.Tensor('float32', imgA, [batchCount, 3, PANEL_CROP_SIZE, PANEL_CROP_SIZE]),
            [panelOrderSession.inputNames[1]]: new ort.Tensor('float32', imgB, [batchCount, 3, PANEL_CROP_SIZE, PANEL_CROP_SIZE]),
            [panelOrderSession.inputNames[2]]: new ort.Tensor('float32', pos, [batchCount, 12]),
        };

        const res = await panelOrderSession.run(feed);
        const logits = res[panelOrderSession.outputNames[0]].data;
        for (let b = 0; b < batchCount; b++) {
            allLogits.push(logits[b]);
        }
    }

    // Sum logits per panel
...
function computePosFeatures(a, b, w, h) {
    ...
    const cxa = ax + aw / 2, cya = ay + ah / 2;
    const cxb = bx + bw / 2, cyb = by + bh / 2;

    return [
        cxa, cya, aw, ah,
        cxb, cyb, bw, bh,
        cxa - cxb, cya - cyb, aw - bw, ah - bh
    ];
}
    ...
    for (const b of bubbles) {
        ...
        if (assigned === -1) {
            ...
            for (let pi = 0; pi < panels.length; pi++) {
                ...
                const ix1 = Math.max(bx1, px1), iy1 = Math.max(by1, py1);
                const ix2 = Math.min(bx2, px2), iy2 = Math.min(by2, py2);
                if (ix1 < ix2 && iy1 < iy2) {
...
// ---------------------------------------------------------------------------
// ReaderNet Poneglyph bubble ordering
// ---------------------------------------------------------------------------
async function sortBubblesWithReaderNet(bubbles, panels, assignments, bitmap) {
    ...
    const normBubbles = bubbles.map(b => ({
        x: b.x / width,
        y: b.y / height,
        w: b.w / width,
        h: b.h / height,
    ...
    // Prepare grayscale page image at 384x256
    const ratio = PAGE_H / height;
    const newW = Math.min(Math.round(width * ratio), PAGE_W);
    const padLeft = Math.floor((PAGE_W - newW) / 2);

    const pageCanvas = new OffscreenCanvas(PAGE_W, PAGE_H);
    const pageCtx = pageCanvas.getContext('2d');
    pageCtx.fillStyle = '#000000';
    pageCtx.fillRect(0, 0, PAGE_W, PAGE_H);
    pageCtx.drawImage(bitmap, padLeft, 0, newW, PAGE_H);

    const pageData = pageCtx.getImageData(0, 0, PAGE_W, PAGE_H);
    const pageGray = new Float32Array(PAGE_W * PAGE_H);
    for (let i = 0; i < PAGE_W * PAGE_H; i++) {
        pageGray[i] = (pageData.data[i * 4] * 0.299 + pageData.data[i * 4 + 1] * 0.587 + pageData.data[i * 4 + 2] * 0.114) / 255.0;
    }

    // Extract bubble crops (grayscale, 96x96)
    const bubbleCrops = [];
    for (const b of normBubbles) {
        const x1 = Math.max(0, Math.round(b.x * width));
        const y1 = Math.max(0, Math.round(b.y * height));
        const x2 = Math.min(width, Math.round((b.x + b.w) * width));
        const y2 = Math.min(height, Math.round((b.y + b.h) * height));

        if (x2 <= x1 || y2 <= y1) {
            bubbleCrops.push(new Float32Array(BUBBLE_CROP_SIZE * BUBBLE_CROP_SIZE).fill(0));
            continue;
        }

        // Use canvas for bilinear resize to 96x96
        const cropCanvas = new OffscreenCanvas(BUBBLE_CROP_SIZE, BUBBLE_CROP_SIZE);
        const cropCtx = cropCanvas.getContext('2d');
        cropCtx.drawImage(bitmap, x1, y1, x2 - x1, y2 - y1, 0, 0, BUBBLE_CROP_SIZE, BUBBLE_CROP_SIZE);
        const cropData = cropCtx.getImageData(0, 0, BUBBLE_CROP_SIZE, BUBBLE_CROP_SIZE);
        const cropGray = new Float32Array(BUBBLE_CROP_SIZE * BUBBLE_CROP_SIZE);
        for (let i = 0; i < BUBBLE_CROP_SIZE * BUBBLE_CROP_SIZE; i++) {
            cropGray[i] = (cropData.data[i * 4] * 0.299 + cropData.data[i * 4 + 1] * 0.587 + cropData.data[i * 4 + 2] * 0.114) / 255.0;
        }
    ...
    const numPanels = panels.length || 1;

    // If no panels, create dummy panel
    const finalPanels = numPanels > 0 ? normPanels : [{ x: 0, y: 0, w: 1, h: 1 }];
    const finalAssignments = assignments.map(a => a < 0 ? 0 : a);

    // Pad to some reasonable max (model has dynamic axes)
    const maxBubbles = Math.max(numBubbles, 1);
    const maxPanels = Math.max(numPanels, 1);

    const geoms = new Float32Array(1 * maxBubbles * 4);
    const bubblePanels = new BigInt64Array(1 * maxBubbles);
    const cropsArr = new Float32Array(1 * maxBubbles * 1 * BUBBLE_CROP_SIZE * BUBBLE_CROP_SIZE);
    const panelsArr = new Float32Array(1 * maxPanels * 4);
    const panelMask = new Uint8Array(1 * maxPanels);
    const bubbleMask = new Uint8Array(1 * maxBubbles);

    for (let i = 0; i < numBubbles; i++) {
        geoms[i * 4 + 0] = normBubbles[i].x;
        geoms[i * 4 + 1] = normBubbles[i].y;
        geoms[i * 4 + 2] = normBubbles[i].w;
        geoms[i * 4 + 3] = normBubbles[i].h;
        bubblePanels[i] = BigInt(finalAssignments[i]);
        bubbleMask[i] = 0;
        for (let j = 0; j < BUBBLE_CROP_SIZE * BUBBLE_CROP_SIZE; j++) {
            cropsArr[i * BUBBLE_CROP_SIZE * BUBBLE_CROP_SIZE + j] = bubbleCrops[i][j];
        }
    }
    for (let i = numBubbles; i < maxBubbles; i++) {
        bubbleMask[i] = 1;
        bubblePanels[i] = BigInt(0);
    }

    for (let i = 0; i < finalPanels.length; i++) {
        panelsArr[i * 4 + 0] = finalPanels[i].x;
        panelsArr[i * 4 + 1] = finalPanels[i].y;
        panelsArr[i * 4 + 2] = finalPanels[i].w;
        panelsArr[i * 4 + 3] = finalPanels[i].h;
        panelMask[i] = 0;
    }
    for (let i = finalPanels.length; i < maxPanels; i++) {
        panelMask[i] = 1;
    }

    const feed = {
        images: new ort.Tensor('float32', pageGray, [1, 1, PAGE_H, PAGE_W]),
        panels: new ort.Tensor('float32', panelsArr, [1, maxPanels, 4]),
        bubbles: new ort.Tensor('float32', geoms, [1, maxBubbles, 4]),
        bubble_panels: new ort.Tensor('int64', bubblePanels, [1, maxBubbles]),
        bubble_crops: new ort.Tensor('float32', cropsArr, [1, maxBubbles, 1, BUBBLE_CROP_SIZE, BUBBLE_CROP_SIZE]),
        panel_mask: new ort.Tensor('bool', panelMask, [1, maxPanels]),
        bubble_mask: new ort.Tensor('bool', bubbleMask, [1, maxBubbles]),
    };
    ...
    const panelGroups = {};
    for (let i = 0; i < numBubbles; i++) {
        const p = finalAssignments[i];
        if (!panelGroups[p]) panelGroups[p] = [];
        panelGroups[p].push(i);
    }

    const sortedIndices = [];
    for (let pi = 0; pi < finalPanels.length; pi++) {
        const group = panelGroups[pi] || [];
        if (group.length === 0) continue;
        // Sort by score ascending (lower score = earlier in reading order)
        group.sort((a, b) => scores[a] - scores[b]);
        sortedIndices.push(...group);
    }

    // Any unassigned bubbles (shouldn't happen with fallback)
    const unassigned = [];
    for (let i = 0; i < numBubbles; i++) {
        if (!sortedIndices.includes(i)) unassigned.push(i);
    }
    if (unassigned.length > 0) {
        unassigned.sort((a, b) => scores[a] - scores[b]);
        sortedIndices.push(...unassigned);
    }
