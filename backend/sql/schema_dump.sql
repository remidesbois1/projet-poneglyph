...
--
-- Name: statut_bulle; Type: TYPE; Schema: public; Owner: postgres
--

CREATE TYPE public.statut_bulle AS ENUM (
    'Proposé',
    'Validé',
    'Rejeté'
);
...
ALTER TYPE public.statut_bulle OWNER TO postgres;
...
--
-- Name: get_pending_bubbles(integer, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.get_pending_bubbles(page_limit integer, page_offset integer) RETURNS TABLE(id bigint, texte_propose text, total_count bigint)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    WITH pending AS (
        SELECT b.id, b.texte_propose
        FROM public.bulles b
        WHERE b.statut = 'Proposé'
        ORDER BY b.created_at ASC
    )
    SELECT 
        p.id,
        p.texte_propose,
        (SELECT COUNT(*) FROM pending) AS total_count
    FROM pending p
    LIMIT page_limit
    OFFSET page_offset;
END;
...
ALTER FUNCTION public.get_pending_bubbles(page_limit integer, page_offset integer) OWNER TO postgres;

--
-- Name: get_summary_stats(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.get_summary_stats() RETURNS TABLE(total_bulles_validees bigint, total_contributeurs bigint, total_pages_completees bigint)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        (SELECT COUNT(*) FROM public.bulles WHERE statut = 'Validé'),
        (SELECT COUNT(DISTINCT id_user_createur) FROM public.bulles WHERE statut = 'Validé'),
        (SELECT COUNT(*) FROM public.pages WHERE statut = 'completed');
END;
$$;


ALTER FUNCTION public.get_summary_stats() OWNER TO postgres;

--
-- Name: get_top_contributors(integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.get_top_contributors(limit_count integer) RETURNS TABLE(user_id uuid, username text, bounty bigint)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        b.id_user_createur AS user_id,
        p.username AS username,
        COUNT(b.id) AS bounty
    FROM public.bulles b
    JOIN public.profiles p ON b.id_user_createur = p.id
    WHERE b.statut = 'Validé'
    GROUP BY b.id_user_createur, p.username
...
ALTER FUNCTION public.get_top_contributors(limit_count integer) OWNER TO postgres;
...
CREATE FUNCTION public.match_pages_hybrid(query_embedding public.vector, query_text text, match_threshold double precision, match_count integer) RETURNS TABLE(id bigint, numero_page integer, url_image text, description jsonb, similarity double precision, base_similarity double precision, chapitre_numero integer, tome_numero integer)
    LANGUAGE plpgsql
    AS $_$
declare
  keywords_regex text;
begin
  -- Construction de la Regex (mots > 2 lettres)
  select string_agg(word, '|')
  into keywords_regex
  from (
      select unnest(string_to_array(trim(query_text), ' ')) as word
  ) t
  where length(word) > 2;

  -- Si pas de mots-clés, on rend le regex impossible
  if keywords_regex is null or keywords_regex = '' then
      keywords_regex := '$.^'; 
  end if;
  ...
  select
    ...
    p.description,
    (
      -- A. Score Vectoriel de base
      (1 - (p.embedding <=> query_embedding)) 
      +
      -- B. LE BOOST (+0.25)
      -- Si un mot-clé est trouvé, on propulse le score.
      (CASE WHEN 
          EXISTS (SELECT 1 FROM bulles b WHERE b.id_page = p.id AND b.texte_propose ~* keywords_regex)
          OR (p.description->>'content' ~* keywords_regex) 
          OR (p.description->'metadata'->>'arc' ~* keywords_regex)
          OR ((p.description->'metadata'->'characters')::text ~* keywords_regex)
      THEN 0.25 ELSE 0 END)
    ) as similarity,
    
    (1 - (p.embedding <=> query_embedding)) as base_similarity,
    c.numero as chapitre_numero,
    t.numero as tome_numero
  from pages p
  join chapitres c on p.id_chapitre = c.id
  join tomes t on c.id_tome = t.id
  -- C. FILTRAGE STRICT
  -- On ne garde que ce qui dépasse le seuil (qui sera fixé haut, ex: 0.78)
  where (
      (1 - (p.embedding <=> query_embedding)) 
      +
      (CASE WHEN 
...
CREATE FUNCTION public.reorder_bubbles(bubbles_data jsonb) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  item jsonb;
BEGIN
  -- On boucle sur chaque élément envoyé par le frontend
  FOR item IN SELECT * FROM jsonb_array_elements(bubbles_data)
  LOOP
    UPDATE bulles
    -- On met à jour la position
    SET "order" = (item->>'order')::INTEGER
    -- POINT CRITIQUE : On force le cast de l'ID en BIGINT (et non UUID)
...
ALTER FUNCTION public.reorder_bubbles(bubbles_data jsonb) OWNER TO postgres;

--
-- Name: search_bulles(text); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.search_bulles(search_term text) RETURNS TABLE(id bigint, texte_propose text, page_id bigint, numero_page integer, chapitre_id bigint, numero_chapitre integer, titre_chapitre character varying, tome_id bigint, numero_tome integer, titre_tome character varying)
    LANGUAGE plpgsql
    AS $$
BEGIN
    RETURN QUERY
    SELECT
        ...
        c.titre AS titre_chapitre,
        t.id AS tome_id,
        t.numero AS numero_tome,
        t.titre AS titre_tome
    FROM
        public.bulles b
    JOIN
        public.pages p ON b.id_page = p.id
    JOIN
        public.chapitres c ON p.id_chapitre = c.id
    JOIN
        public.tomes t ON c.id_tome = t.id
    WHERE
        b.statut = 'Validé'
        -- LA CORRECTION EST SUR LA LIGNE SUIVANTE :
        AND b.fts @@ websearch_to_tsquery('french', search_term);
END;
$$;


ALTER FUNCTION public.search_bulles(search_term text) OWNER TO postgres;

--
-- Name: search_bulles(text, integer, integer); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.search_bulles(search_term text, page_limit integer, page_offset integer) RETURNS TABLE(id bigint, texte_propose text, numero_page integer, chapitre_numero integer, tome_numero integer, url_image text, page_id bigint, x integer, y integer, w integer, h integer, total_count bigint)
    LANGUAGE plpgsql
    AS $$
begin
  return query
  ...
  from bulles b
  join pages p on b.id_page = p.id
  join chapitres c on p.id_chapitre = c.id
  join tomes t on c.id_tome = t.id
  where b.texte_propose ilike '%' || search_term || '%'
...
ALTER FUNCTION public.search_bulles(search_term text, page_limit integer, page_offset integer) OWNER TO postgres;
...
--
-- Name: bubble_history; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.bubble_history (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
ALTER TABLE public.bubble_history OWNER TO postgres;

--
-- Name: bulles; Type: TABLE; Schema: public; Owner: postgres
--

CREATE TABLE public.bulles (
    id bigint NOT NULL,
    id_page bigint NOT NULL,
    id_user_createur uuid NOT NULL,
    x integer NOT NULL,
    y integer NOT NULL,
    w integer NOT NULL,
    h integer NOT NULL,
    texte_ocr_brut text,
    texte_propose text,
    statut public.statut_bulle DEFAULT 'Proposé'::public.statut_bulle,
    created_at timestamp with time zone DEFAULT now(),
    validated_at timestamp with time zone,
    fts tsvector GENERATED ALWAYS AS (to_tsvector('french'::regconfig, texte_propose)) STORED,
    "order" integer,
    commentaire_moderation text
);


ALTER TABLE public.bulles OWNER TO postgres;

--
-- Name: bulles_id_seq; Type: SEQUENCE; Schema: public; Owner: postgres
--

ALTER TABLE public.bulles ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.bulles_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: chapitres; Type: TABLE; Schema: public; Owner: postgres
--
...
ALTER TABLE public.chapitres OWNER TO postgres;

--
-- Name: chapitres_id_seq; Type: SEQUENCE; Schema: public; Owner: postgres
--

ALTER TABLE public.chapitres ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.chapitres_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE public.mangas ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
...
--
-- Name: search_feedback; Type: TABLE; Schema: public; Owner: postgres
--
...
ALTER TABLE public.tomes OWNER TO postgres;

--
-- Name: tomes_id_seq; Type: SEQUENCE; Schema: public; Owner: postgres
--

ALTER TABLE public.tomes ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.tomes_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: bubble_history bubble_history_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.bubble_history
    ADD CONSTRAINT bubble_history_pkey PRIMARY KEY (id);


--
-- Name: bulles bulles_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.bulles
    ADD CONSTRAINT bulles_pkey PRIMARY KEY (id);


--
-- Name: chapitres chapitres_id_tome_numero_key; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chapitres
    ADD CONSTRAINT chapitres_id_tome_numero_key UNIQUE (id_tome, numero);


--
-- Name: chapitres chapitres_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: mangas mangas_slug_key; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: pages pages_id_chapitre_numero_page_key; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.pages
    ADD CONSTRAINT pages_id_chapitre_numero_page_key UNIQUE (id_chapitre, numero_page);
...
--
-- Name: search_feedback search_feedback_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: tomes tomes_manga_unique; Type: CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.tomes
    ADD CONSTRAINT tomes_manga_unique UNIQUE (manga_id, numero);


--
-- Name: tomes tomes_pkey; Type: CONSTRAINT; Schema: public; Owner: postgres
--
...
CREATE INDEX bulles_fts_idx ON public.bulles USING gin (fts);


--
-- Name: idx_bubble_history_bubble_id; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX idx_bubble_history_bubble_id ON public.bubble_history USING btree (bubble_id);


--
-- Name: idx_bulles_texte_fts; Type: INDEX; Schema: public; Owner: postgres
--

CREATE INDEX idx_bulles_texte_fts ON public.bulles USING gin (to_tsvector('french'::regconfig, texte_propose));
...
ALTER TABLE ONLY public.bubble_history
    ADD CONSTRAINT bubble_history_bubble_id_fkey FOREIGN KEY (bubble_id) REFERENCES public.bulles(id) ON DELETE CASCADE;


--
-- Name: bubble_history bubble_history_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.bubble_history
    ADD CONSTRAINT bubble_history_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);


--
-- Name: bulles bulles_id_page_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.bulles
    ADD CONSTRAINT bulles_id_page_fkey FOREIGN KEY (id_page) REFERENCES public.pages(id) ON DELETE CASCADE;


--
-- Name: bulles bulles_id_user_createur_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.bulles
    ADD CONSTRAINT bulles_id_user_createur_fkey FOREIGN KEY (id_user_createur) REFERENCES auth.users(id);


--
-- Name: chapitres chapitres_id_tome_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.chapitres
    ADD CONSTRAINT chapitres_id_tome_fkey FOREIGN KEY (id_tome) REFERENCES public.tomes(id) ON DELETE CASCADE;


--
-- Name: pages pages_id_chapitre_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.pages
    ADD CONSTRAINT pages_id_chapitre_fkey FOREIGN KEY (id_chapitre) REFERENCES public.chapitres(id) ON DELETE CASCADE;
...
--
-- Name: search_feedback search_feedback_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--
...
--
-- Name: tomes tomes_manga_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.tomes
    ADD CONSTRAINT tomes_manga_id_fkey FOREIGN KEY (manga_id) REFERENCES public.mangas(id);
...
--
-- Name: bubble_history Enable insert for authenticated users; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable insert for authenticated users" ON public.bubble_history FOR INSERT WITH CHECK ((auth.role() = 'authenticated'::text));


--
-- Name: bubble_history Enable read access for authenticated users; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Enable read access for authenticated users" ON public.bubble_history FOR SELECT USING ((auth.role() = 'authenticated'::text));
...
--
-- Name: search_feedback Users can insert their own feedback; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can insert their own feedback" ON public.search_feedback FOR INSERT TO authenticated WITH CHECK ((auth.uid() = user_id));


--
-- Name: search_feedback Users can view their own feedback; Type: POLICY; Schema: public; Owner: postgres
--

CREATE POLICY "Users can view their own feedback" ON public.search_feedback FOR SELECT TO authenticated USING ((auth.uid() = user_id));
...
--
-- Name: FUNCTION get_pending_bubbles(page_limit integer, page_offset integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.get_pending_bubbles(page_limit integer, page_offset integer) TO anon;
GRANT ALL ON FUNCTION public.get_pending_bubbles(page_limit integer, page_offset integer) TO authenticated;
GRANT ALL ON FUNCTION public.get_pending_bubbles(page_limit integer, page_offset integer) TO service_role;


--
-- Name: FUNCTION get_summary_stats(); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.get_summary_stats() TO anon;
GRANT ALL ON FUNCTION public.get_summary_stats() TO authenticated;
GRANT ALL ON FUNCTION public.get_summary_stats() TO service_role;


--
-- Name: FUNCTION get_top_contributors(limit_count integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.get_top_contributors(limit_count integer) TO anon;
GRANT ALL ON FUNCTION public.get_top_contributors(limit_count integer) TO authenticated;
GRANT ALL ON FUNCTION public.get_top_contributors(limit_count integer) TO service_role;
...
GRANT ALL ON FUNCTION public.match_pages(query_embedding public.vector, match_threshold double precision, match_count integer) TO anon;
GRANT ALL ON FUNCTION public.match_pages(query_embedding public.vector, match_threshold double precision, match_count integer) TO authenticated;
GRANT ALL ON FUNCTION public.match_pages(query_embedding public.vector, match_threshold double precision, match_count integer) TO service_role;
...
--
-- Name: FUNCTION reorder_bubbles(bubbles_data jsonb); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.reorder_bubbles(bubbles_data jsonb) TO anon;
GRANT ALL ON FUNCTION public.reorder_bubbles(bubbles_data jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.reorder_bubbles(bubbles_data jsonb) TO service_role;


--
-- Name: FUNCTION search_bulles(search_term text); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.search_bulles(search_term text) TO anon;
GRANT ALL ON FUNCTION public.search_bulles(search_term text) TO authenticated;
GRANT ALL ON FUNCTION public.search_bulles(search_term text) TO service_role;


--
-- Name: FUNCTION search_bulles(search_term text, page_limit integer, page_offset integer); Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON FUNCTION public.search_bulles(search_term text, page_limit integer, page_offset integer) TO anon;
GRANT ALL ON FUNCTION public.search_bulles(search_term text, page_limit integer, page_offset integer) TO authenticated;
GRANT ALL ON FUNCTION public.search_bulles(search_term text, page_limit integer, page_offset integer) TO service_role;
...
--
-- Name: TABLE banned_ips; Type: ACL; Schema: public; Owner: postgres
--
...
--
-- Name: TABLE bubble_history; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON TABLE public.bubble_history TO anon;
GRANT ALL ON TABLE public.bubble_history TO authenticated;
GRANT ALL ON TABLE public.bubble_history TO service_role;


--
-- Name: TABLE bulles; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON TABLE public.bulles TO anon;
GRANT ALL ON TABLE public.bulles TO authenticated;
GRANT ALL ON TABLE public.bulles TO service_role;


--
-- Name: SEQUENCE bulles_id_seq; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON SEQUENCE public.bulles_id_seq TO anon;
GRANT ALL ON SEQUENCE public.bulles_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.bulles_id_seq TO service_role;


--
-- Name: TABLE chapitres; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON TABLE public.chapitres TO anon;
GRANT ALL ON TABLE public.chapitres TO authenticated;
GRANT ALL ON TABLE public.chapitres TO service_role;


--
-- Name: SEQUENCE chapitres_id_seq; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON SEQUENCE public.chapitres_id_seq TO anon;
GRANT ALL ON SEQUENCE public.chapitres_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.chapitres_id_seq TO service_role;
...
--
-- Name: TABLE search_feedback; Type: ACL; Schema: public; Owner: postgres
--
...
--
-- Name: TABLE tomes; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON TABLE public.tomes TO anon;
GRANT ALL ON TABLE public.tomes TO authenticated;
GRANT ALL ON TABLE public.tomes TO service_role;


--
-- Name: SEQUENCE tomes_id_seq; Type: ACL; Schema: public; Owner: postgres
--

GRANT ALL ON SEQUENCE public.tomes_id_seq TO anon;
GRANT ALL ON SEQUENCE public.tomes_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.tomes_id_seq TO service_role;
